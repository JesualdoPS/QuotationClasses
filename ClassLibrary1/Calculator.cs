...
namespace ClassLibrary1
{
    public class Calculator
    {
        ...
        {
            var serializedMemory = Memory.Select(mathLog => new
            {
                mathLog.Math,
                Result = mathLog.ResultToString()
            }).ToList();

            var json = JsonSerializer.Serialize(serializedMemory);
        ...
        public void LoadMemory(string filePath)
        {
            ...
            {
                var mathLog = MathLog.FromString(item.Math, item.Result.ToString());
                Memory.Add(mathLog);
            }
        ...
        private MathLog EvaluateAndCalculate(string input)
        {
            ...
            if (!HasRecognizedAll(input, matchingParts)) throw new NotImplementedException();
